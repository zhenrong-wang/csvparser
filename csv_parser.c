...
int csv_parser(const char *data_file, char *search_kwd, char **matched_list, const size_t matched_list_max) {
    ...
    size_t matched_line_num = 0;
    struct stat file_stat;
    if(fstat(fd, &file_stat) == -1) {
        ret_flag = ERR_FILE_STAT;
        goto clean_and_return;
    }
    char *map_head = mmap(NULL, file_stat.st_size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0); 
    ...
    for(size_t pos = 0; pos < file_stat.st_size; pos++) {
        if(map_head[pos] == '\n') {
            map_head[pos] = '\0';
            if(kmp_search_fast(p_tmp, search_kwd, next_array) >= 0) {
                char *line_tmp = (char *)calloc(pos - idx_tmp + 1, sizeof(char));
                if(line_tmp == NULL) {
                    ret_flag = ERR_ALLOC_IN_LOOP;
                    break;
                }
                memcpy(line_tmp, p_tmp, pos - idx_tmp);
